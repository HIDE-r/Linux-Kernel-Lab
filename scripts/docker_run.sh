...
notify-send-wsl()
{
	if which wsl-notify-send.exe > /dev/null 2>&1; then
...
if [ "$#" = "0" ];then
	run docker run --rm "$DOCKER_TTY" "${DOCKER_OPTS[@]}" "${DOCKER_IAMGE}" bash
else
	run docker run --rm "$DOCKER_TTY" "${DOCKER_OPTS[@]}" "${DOCKER_IAMGE}" bash -c "$*"
	bell
fi
